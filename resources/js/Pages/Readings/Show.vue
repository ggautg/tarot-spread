...
<template>
  ...
  <div class="min-h-screen relative bg-[#150F26] text-[#EDE3D0] px-6 py-10">
    ...
    <div class="max-w-3xl mx-auto text-center mb-4 pt-14">
      ...
      <p v-if="reading.pregunta" class="mt-2 text-[#EDE3D0]/60 italic"
        style="font-family: 'Cormorant Garamond', serif;">
        "{{ reading.pregunta }}"
      </p>
      <p class="mt-3 text-[10px] font-mono text-[#EDE3D0]/25 break-all">
        seed: {{ reading.seed.substring(0, 32) }}...
      </p>
    ...
    <div class="relative max-w-3xl mx-auto aspect-[4/3]">
      <div v-for="rc in reading.cards" :key="rc.id" class="absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer"
        :style="{
        ...
        <div class="flip-card" :class="{ flipped: isRevealed(rc.id) }">
          <div class="flip-inner">
            ...
            <div class="flip-face flip-back">
              <div
                class="w-full h-full rounded border-2 border-[#C9A227]/60 bg-[#4A3B6B] flex items-center justify-center">
                <span class="text-2xl text-[#C9A227]">✦</span>
              </div>
            </div>
            <!-- Frente -->
            <div class="flip-face flip-front">
              <div class="w-full h-full rounded border-2 p-2 flex flex-col justify-between text-center"
    ...
    <div class="max-w-2xl mx-auto mt-16 space-y-6">
      <div v-for="rc in reading.cards.filter(c => isRevealed(c.id))" :key="'detail-' + rc.id"
        ...
        <p style="font-family: 'Cormorant Garamond', serif; font-size: 1.1rem;"
          class="text-[#EDE3D0]/80 leading-relaxed">
